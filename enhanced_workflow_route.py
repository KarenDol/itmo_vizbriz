"""
Enhanced Patient Workflow Route
This integrates the enhanced patient workflow with the existing Flask application.
"""

from flask import Blueprint, render_template, jsonify, request, flash, redirect, url_for
from flask_login import login_required, current_user
from enhanced_patient_workflow import get_enhanced_patient_workflow
import logging
...
# Create blueprint
enhanced_workflow = Blueprint('enhanced_workflow', __name__)

@enhanced_workflow.route('/enhanced_patient_workflow/<int:patient_id>', methods=['GET'])
@login_required
def enhanced_patient_workflow_page(patient_id):
    """Enhanced patient workflow page with unified manifest validation and file links"""
    try:
        ...
        return render_template('enhanced_patient_journey.html',
                             patient=patient,
        ...
        logger.error(f"Error in enhanced patient workflow: {e}")
        flash(f'Error loading enhanced patient workflow: {str(e)}', 'error')
        return redirect(url_for('main.patient_list'))

@enhanced_workflow.route('/api/enhanced_workflow/<int:patient_id>', methods=['GET'])
@login_required
def enhanced_workflow_api(patient_id):
    """API endpoint for enhanced workflow data"""
    try:
        workflow_data = get_enhanced_patient_workflow(patient_id)
        return jsonify(workflow_data)
    except Exception as e:
        logger.error(f"Error in enhanced workflow API: {e}")
        return jsonify({'error': str(e)}), 500

@enhanced_workflow.route('/api/stage_files/<int:patient_id>/<stage_key>', methods=['GET'])
@login_required
def stage_files_api(patient_id, stage_key):
    """API endpoint for stage-specific files"""
    try:
        from enhanced_patient_workflow import get_file_links_for_stage
